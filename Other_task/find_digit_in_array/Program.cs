...
{ // метод для поиска числа в массиве, передаем массив и искомое число
    int result = 0; // результат работы метода, 0 не найдено, 1 найдено
...
Console.Clear();
int FindDigit = Prompt($"Введите кискомое число: ");// задаём число которое нужно найти в массиве
Console.Clear();


int[] RandomArray = GetRandomArray(len, start, end); // вызываем метод для генерации массива

int res = Find(RandomArray,FindDigit); // вызываем метод поиска числа и помещаем результат в переменную
...
{
    System.Console.WriteLine($"Сгенерирован массив: длинна {len}");
    System.Console.WriteLine($"искомое число: {FindDigit}");
...
if (res == 1){
    ...
    System.Console.WriteLine($"Искомое число ({FindDigit}) найденно в массиве");
} else {
    System.Console.WriteLine("");
    System.Console.WriteLine($"Искомое число ({FindDigit}) НЕ найденно в массиве"); 
}